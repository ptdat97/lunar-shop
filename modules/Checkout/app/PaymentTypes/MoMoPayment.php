...
/**
 * MoMo payment driver. Like VNPay, MoMo is redirect-based, so "authorize" only
 * turns the cart into an order in the awaiting-payment state — the actual
 * payment is confirmed later by the MoMo return/IPN callback (which records the
 * Transaction and moves the order to payment-received). The payUrl is created by
 * the storefront from the resulting order via MoMoGateway.
 */
class MoMoPayment extends AbstractPayment
{
