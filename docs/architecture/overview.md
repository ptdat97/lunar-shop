...
> lỗi im lặng.** Năm ca có thật trong repo này, tất cả đều chạy đúng cho tới
> ngày không đúng nữa:
>
...
> | `TokenAwareCartSession::createNewCart()` dựng `Cart::create()` bằng tay | `ResolveStorefrontContext` — Lunar gọi nó là *"the single home for the storefront default cascade"* | Giỏ của client stateless lệch giỏ web ở **ba trường**: `region_id` để NULL, channel và currency lấy mặc định **toàn cục** thay vì của region. `FillOrderFromCart` chép `region_id` thẳng sang đơn → mọi đơn đặt từ app mang region rỗng. Chưa nổ vì shop một region trỏ đúng vào mặc định (2026-09-18) |
>
...
| 29 | 2026-09-18 | **Rà một lượt theo Nguyên tắc số 0** — soi `modules/` tìm chỗ tự viết thứ Lunar đã có. Cách soi là cách chính doc chỉ: đọc `vendor/` xem Lunar lấy quyết định TỪ ĐÂU, không đọc code của mình. Tìm ra **một lệch thật**: `TokenAwareCartSession::createNewCart()` dựng `Cart::create()` bằng tay trong khi `parent::` đi qua `ResolveStorefrontContext`, nên giỏ của client stateless để `region_id` NULL và lấy channel/currency mặc định **toàn cục** thay vì của region (`getCurrency()`/`getChannel()` bỏ qua hẳn tầng region). `FillOrderFromCart` chép `region_id` sang đơn → đơn đặt từ app mang region rỗng. Sửa bằng cách gọi đúng resolver đó, giữ nguyên hai thứ bản ghi đè sinh ra để làm: đọc user theo guard sanctum, và mint `public_token`. `CustomerResolver` cũng chuyển sang `CreatesCustomer` — không đổi hành vi, chỉ để storefront và panel dùng CHUNG một seam | 886 test (từ 882); 4 test mới đỏ TRƯỚC khi sửa, xanh sau — `region_id` null, currency sai cascade, hai cửa vào lệch nhau. **Phần còn lại của lượt rà là sạch, ghi ra để khỏi soi lại:** guard tồn kho đọc `getTotalInventory()` (không đọc `stock_on_hand`), hoàn tiền qua `RefundsOrder`, không ai ghi thẳng `orders.status`, không chỗ nào tự tính tổng tiền hay tự resolve giá, `thumbnail` dùng MorphOne của Lunar chứ không back-fill tay. `MediaLibraryService` CỐ Ý không dùng `AddsMedia`: action đó không nhận `usingFileName`/`usingName`/`preservingOriginal` nên dùng nó là MẤT hành vi — đúng "chiều ngược lại cũng là một cái bẫy" |
