...
| **Review** | Đánh giá sản phẩm (rating + nội dung) | — | ✅ model + service + API (`products/{product}/reviews`); summary (count+average) nhúng vào product payload |
| **Recommend** | Gợi ý "You may also like" (curate + collection fallback) | `ProductAssociation` của Lunar | ✅ strategy chain (Association → Collection) + API; product page/API gọi service trực tiếp |
