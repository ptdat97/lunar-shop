...
| **Hook** | action/filter (Eventy) + domain events liên-module | Lunar events | ✅ `HookManager` native (action/filter theo priority) + facade `Hook` + registry `Hooks::*`. **Hook hoá toàn bộ payload + event**: FILTER trên mọi API Resource (`product.resource`/`cart.resource`/`collection.resource`/`order.resource`/`search.results`) cho phép module khác enrich payload không cần phụ thuộc; FILTER `product.purchasable` (Inventory veto oversell); ACTION `order.placed`/`order.paid`/`order.status_changed`. Consumer thật: Inventory thêm block `availability` vào product payload qua `product.resource`. ⬜ **Đề xuất mở rộng (2026-06-26):** bộ domain events đầy đủ (cart/checkout/customer/catalog/inventory/promotion) + **Plugin SDK** (contract `Plugin` + `PluginManager` + lifecycle) — xem mục **"Domain Events + Plugin SDK"** |
...
# Domain Events + Plugin SDK — chuẩn bị hệ sinh thái mở rộng (ĐỀ XUẤT 2026-06-26)

> **Mục tiêu:** cho **dev (team nội bộ hoặc đối tác kỹ thuật)** một cách **mở rộng hệ thống
> bằng plugin nội bộ** — viết một **plugin** drop-in để thêm tính năng (loyalty, review,
> affiliate, ERP sync, gateway mới…) **mà không sửa code lõi**. Hai trụ cột:
>
> 1. **Domain events đầy đủ** — phủ kín vòng đời nghiệp vụ (catalog, cart, checkout,
>    order, customer, inventory, promotion) để plugin có đủ điểm móc.
> 2. **Plugin SDK** — một contract + lifecycle + discovery chuẩn để plugin **tự đăng ký,
>    tự khai báo phụ thuộc, tự chứa** (routes/migrations/views/Filament/hooks).
>
> 🎯 **Phạm vi rõ ràng — đây là cơ chế MỞ RỘNG NỘI BỘ, KHÔNG phải nền tảng SaaS.**
> Plugin để **dev của ta (hoặc đối tác làm thuê)** thêm/đổi tính năng cho **một shop duy
> nhất** đang vận hành — giống cách WordPress/Lunar cho extend bằng package. **KHÔNG** nhằm:
> bán phần mềm dạng dịch vụ (SaaS), multi-tenant (một codebase phục vụ nhiều shop độc lập),
> self-serve sign-up, hay sàn phân phối plugin cho người ngoài cài. Một deploy = một shop;
> plugin chỉ là cách tổ chức code mở rộng cho deploy đó.

> ⚠️ **Nguyên tắc #1 vẫn tối thượng.** Hạ tầng dưới đây **kế thừa thẳng** `HookManager`
> + registry `Hooks::*` đã có (xem module **Hook**) và mẫu `AdminPages`/`ModulesServiceProvider`
> đã chạy — **không** thay thế, chỉ **mở rộng**. Không kéo vendor plugin-framework (`nwidart/
> laravel-modules`, `composer-plugin`…) khi `HookManager` + service provider thuần đã đủ.

> ⛔ **KHÔNG phải "app store", KHÔNG phải SaaS.** "Những thứ KHÔNG nên build sớm" đã loại
> **marketplace / app store công khai**. Plugin SDK ở đây là **SDK nội bộ cho dev** (first-party
> + module tin cậy), cài qua composer/path, bật bằng config trong repo — để **chính dev của ta**
> mở rộng nhanh. **Không** phải: sàn cài đặt cho người dùng cuối, SaaS/multi-tenant, billing,
> sandbox cô lập code lạ, hay UI marketplace. Plugin chạy **cùng quyền với code lõi** (vì do
> dev tin cậy viết) → vì thế dùng **allow-list trong config**, không tự kích hoạt code lạ.

## A. Hiện trạng (đã có — điểm xuất phát)

- ✅ **`HookManager`** native (action/filter theo priority) + facade `Hook` + registry
  **`Hooks::*`** (constant cho mọi tên hook → producer/consumer không lệch chuỗi).
- ✅ **FILTER trên mọi API Resource** (`product.resource`/`cart.resource`/`collection.resource`/
  `order.resource`/`search.results`) + `product.purchasable` + `menu.items`.
- ✅ **3 ACTION** vòng đời order: `order.placed` / `order.paid` / `order.status_changed`.
- ✅ **Module tự đăng ký** qua `<Name>ServiceProvider` (routes/migrations/views/bindings).
- ✅ **`AdminPages`** registry — module đóng góp Filament page/resource vào panel Lunar.
- ⚠️ **Thiếu để thành "platform":**
  - Domain events **thưa** — mới có order; chưa có cart/customer/catalog/inventory/promotion.
  - Danh sách module **hardcode** trong `ModulesServiceProvider::$modules` → plugin bên thứ ba
    không tự "drop-in" được, phải sửa core.
  - Không có **contract plugin** (metadata, version, phụ thuộc, enable/disable, lifecycle
    install/activate/deactivate).
  - Hook events **chưa có schema/payload versioning** → plugin dễ vỡ khi payload đổi.

## B. Trụ cột 1 — Domain Events đầy đủ

> Quy ước giữ nguyên mẫu đã có: **ACTION** (`<domain>.<event>`) cho "đã xảy ra việc gì"
> (side-effect, không trả giá trị); **FILTER** cho "cho module khác chỉnh giá trị này".
> Tất cả thêm vào registry **`Hooks::*`** (không bao giờ dùng chuỗi trần). Producer là
> **service lõi** đang sở hữu nghiệp vụ (không rải `doAction` khắp controller).

### Bộ event đề xuất (bổ sung — theo độ ưu tiên ROI)

| Domain | Event mới (ACTION trừ khi ghi FILTER) | Producer (service) | Plugin tiêu biểu dùng |
|---|---|---|---|
| **Cart** | `cart.line_added`, `cart.line_updated`, `cart.line_removed`, `cart.coupon_applied`, `cart.emptied` | `CartService` | abandoned-cart, analytics, upsell |
| **Checkout** | `checkout.started`, `checkout.address_set`, `checkout.shipping_selected`, FILTER `checkout.payment_methods` | `CheckoutService` | gateway mới, fraud-check, COD-fee |
| **Order** | (đã có placed/paid/status) + `order.shipped`, `order.cancelled`, `order.refunded`, `order.completed` | `OrderService` | loyalty, ERP sync, RMA, kế toán |
| **Customer** | `customer.registered`, `customer.logged_in`, `customer.address_changed` | `CustomerService` | CRM, welcome-flow, segmentation |
| **Catalog** | `product.viewed`, `product.created`, `product.updated`, FILTER `product.related` | `ProductService` (+ observer) | also-viewed (Recommend P3), review, feed sync |
| **Inventory** | `inventory.low_stock`, `inventory.out_of_stock`, `inventory.restocked` | `InventoryService` | notify-me (đã có consumer), purchasing |
| **Promotion** | `promotion.applied`, `promotion.flash_sale_started`, FILTER `promotion.eligible` | `PromotionService` | affiliate, gam-ified coupon |
| **Search** | (đã có FILTER `search.results`) + `search.performed` | `SearchEngine` wrapper | analytics, "no-result" reporting |

> **Lưu ý kế thừa:** nhiều event này **map thẳng** lên event/observer Lunar đã có
> (Lunar bắn `CartLinesUpdated`, `OrderPaymentEvent`, model `*Observer`…). Producer
> **re-broadcast** chúng qua `HookManager` để có **một mặt phẳng hook thống nhất** cho
> plugin (plugin chỉ học `Hooks::*`, không phải biết cả event Lunar lẫn hook ta).
> Kiểm tra `vendor/lunarphp` trước (Nguyên tắc #1): có event → bắc cầu; không → tự bắn.

### Payload chuẩn hoá + versioning (chống vỡ plugin)

- Mỗi event mang **một DTO immutable** (`Modules\Hook\Events\<Name>Event` hoặc array shape
  ổn định) thay vì model trần → plugin không phụ thuộc cấu trúc DB nội bộ.
- DTO bọc model lõi qua **getter** (`->order()`, `->customer()`), thêm field là **additive**.
- Registry `Hooks::*` giữ **docblock payload** (đã làm cho 8 hook hiện tại) — bắt buộc cho
  mọi event mới: ghi rõ Value + Args để plugin dev đọc một nơi.

## C. Trụ cột 2 — Plugin SDK

> Plugin = **một service provider tuân theo contract `Plugin`** + tự chứa
> (routes/migrations/views/Filament/hooks/config). Cài qua **composer package** hoặc thư mục
> `plugins/<Vendor>/<Name>/`; bật/tắt qua `config/plugins.php` (allow-list — **không** auto-run
> code lạ). SDK chỉ là **một interface mỏng + manager**, không phải framework nặng.

### Contract + lifecycle

```php
namespace Modules\Hook\Plugin;          // SDK sống trong module Hook (đã là "seam" mở rộng)

interface Plugin
{
    public function id(): string;            // 'acme/loyalty' — duy nhất, ổn định
    public function version(): string;       // semver
    public function requires(): array;        // ['core' => '^1.0', 'acme/points' => '^2.0']

    public function register(Container $app): void;   // bind service (giai đoạn register)
    public function boot(HookManager $hooks): void;   // addAction/addFilter vào Hooks::*

    // Lifecycle (idempotent — gọi qua artisan, KHÔNG tự chạy lúc boot):
    public function install(): void;     // migrate, seed config mặc định
    public function activate(): void;    // bật: publish asset, cache route
    public function deactivate(): void;  // tắt: gỡ schedule, giữ data
    public function uninstall(): void;   // gỡ hẳn: rollback migration (có xác nhận)
}
```

```php
// plugin.json (manifest — đọc không cần load PHP, để discovery + admin liệt kê)
{
  "id": "acme/loyalty",
  "name": "Loyalty Points",
  "version": "1.2.0",
  "provider": "Acme\\Loyalty\\LoyaltyPlugin",
  "requires": { "core": "^1.0" },
  "hooks": ["order.paid", "customer.registered"]   // khai báo để admin thấy ảnh hưởng
}
```

### Discovery + an toàn

- **`PluginManager`** quét: (a) composer packages có `extra.lunar-sme.plugin` trong
  `composer.json`, (b) thư mục `plugins/*/*/plugin.json`. Chỉ **load plugin có trong
  allow-list `config/plugins.php`** → drop-in nhưng không tự kích hoạt code chưa duyệt.
- **Thứ tự nạp theo `requires`** (topological sort) — thiếu phụ thuộc/lệch version →
  **báo lỗi rõ, skip plugin**, không làm sập app. `core` version = version app.
- `ModulesServiceProvider` giữ nguyên cho **module first-party** (lõi); `PluginManager`
  chạy **sau** module lõi (plugin móc vào hook do lõi cung cấp).

### Điểm mở rộng plugin được dùng (tái dùng hạ tầng sẵn)

| Cần | Cơ chế (đã có) |
|---|---|
| Thêm nghiệp vụ phản ứng event | `Hook::addAction/addFilter` vào `Hooks::*` (Trụ cột 1) |
| Thêm endpoint API/web | `loadRoutesFrom` trong `boot()` của plugin provider |
| Thêm bảng | `loadMigrationsFrom` + lifecycle `install()` |
| Thêm trang/resource admin | `AdminPages::register()` (registry Filament đã có) |
| Thêm view/section storefront | namespace `theme::` + SectionBuilder partial |
| Thêm payment/shipping/search driver | `Payments::extend` / `ShippingZoneResolver` / `SearchEngine` (đã là interface) |

### CLI quản trị plugin

```text
php artisan plugin:list                 # liệt kê + trạng thái (active/inactive/lỗi requires)
php artisan plugin:install acme/loyalty # migrate + activate (idempotent)
php artisan plugin:disable acme/loyalty # deactivate, giữ data
php artisan plugin:uninstall acme/loyalty --force   # rollback (xác nhận)
```

## D. Lộ trình thực hiện (incremental — không big-bang, mỗi bước có test)

> Quy tắc xuyên suốt: **additive, không đổi URL/return shape đang chạy**; chạy
> `php artisan test` sau mỗi bước (đã có 120 test làm lưới an toàn).

**E0 — Mở rộng domain events (nền cho cả hai trụ cột).** *module Hook + service lõi*
  - Thêm constant vào `Hooks::*` (docblock payload đầy đủ) cho bộ event ở §B, **theo cụm
    ROI**: (1) Order (`shipped/cancelled/refunded/completed`) + Customer (`registered/
    logged_in`) → loyalty/RMA/CRM; (2) Cart/Checkout → abandoned-cart/gateway; (3) Catalog/
    Inventory/Promotion.
  - ✅ **Cụm 1 đã làm (2026-06-26):** `Hooks::ORDER_SHIPPED` (bắn từ `OrderObserver` khi
    status → `dispatched`, kèm `$previousStatus`), `Hooks::CUSTOMER_REGISTERED` +
    `Hooks::CUSTOMER_LOGGED_IN` (bắc cầu event Laravel `Registered`/`Login` qua một listener
    trung tâm trong `CustomerServiceProvider` → bắt mọi path web SPA + API token, controller
    không bị rải side-effect). Registration giờ `event(new Registered($user))` ở cả 2 path.
    +3 test trong `HookTest` (123 test xanh). ⬜ `cancelled/refunded/completed` chờ khi
    cấu hình status tương ứng (Lunar mới có `dispatched`).
  - ✅ **Cụm 2 đã làm (2026-06-26):** Cart events `cart.line_added/line_updated/line_removed/
    coupon_applied/emptied` (bắn từ `CartService` — nguồn cart duy nhất) + Checkout
    `checkout.address_set`/`checkout.shipping_selected` (bắn từ `CheckoutService`). **FILTER
    `checkout.payment_methods`** thành **nguồn duy nhất** danh sách payment method:
    `CheckoutService::paymentMethods()` áp filter lên default `['cod','bank-transfer','vnpay']`,
    validation 2 controller (storefront + API) đọc qua `Rule::in($this->checkout->paymentMethods())`
    → plugin thêm gateway (vd MoMo) chỉ cần append vào filter, **không sửa core**; default
    không đổi (hành vi cũ y nguyên). Bỏ `checkout.started` (storefront single-POST không có
    bước "bắt đầu" tách bạch — `address_set` là tín hiệu vào-checkout cho abandoned-cart).
    +3 test (126 test xanh).
  - ✅ **Cụm 3 đã làm (2026-06-26):** Catalog — `product.viewed` (bắn từ 2 controller detail
    storefront + API, **không** từ mọi `findBySlug` → mở khoá Recommend P3 AlsoViewed),
    `product.created`/`product.updated` (bắc cầu qua `ProductObserver` mới, phủ admin/import/
    seeder), FILTER `product.related` (trong `ProductService::related()` → plugin re-rank/swap
    không cần sửa caller). Inventory — `inventory.low_stock`/`out_of_stock`/`restocked` bắn từ
    `ProductVariantObserver` đã có (nguồn duy nhất phát hiện đổi stock; chỉ bắn khi **vượt
    ngưỡng** xuống/lên để không spam mỗi lần giảm). Search — `search.performed` (term +
    total) qua trait `BroadcastsSearch` dùng chung 2 controller, **chỉ bắn khi có từ khoá**
    (browse rỗng không bắn). +5 test (131 test xanh). ⬜ **Hoãn có chủ đích:** Promotion
    `flash_sale_started` (cần scheduler/time-trigger, không có producer đồng bộ sạch),
    `promotion.applied` (trùng `cart.coupon_applied`), FILTER `promotion.eligible` (trùng
    `appliesToProduct` hiện có) — làm khi thực sự có consumer.

> ✅ **E0 hoàn tất (2026-06-26)** — 3 cụm, **15 hook mới** (`Hooks::*`) phủ order/customer/
> cart/checkout/catalog/inventory/search + 2 FILTER (`checkout.payment_methods`,
> `product.related`). Tất cả bắn từ **service/observer lõi** (không rải controller, trừ
> `product.viewed` vốn là sự kiện trình bày), bắc cầu event Lunar/Laravel sẵn có khi có
> (Nguyên tắc #1). **131 test xanh**, thuần additive. Đủ điểm móc cho Trụ cột 2 (Plugin SDK)
> và cho consumer thật ngay (abandoned-cart, AlsoViewed, analytics, loyalty/CRM).
  - Bắn từ **service lõi** (không phải controller). Ưu tiên **bắc cầu event Lunar** có sẵn
    (kiểm tra `vendor/lunarphp` trước). DTO immutable cho payload mới.
  - Test: mỗi event có 1 feature test "service X → `Hook::doAction(Hooks::Y)` được gọi với
    payload đúng" (dùng `HookManager::forget` + spy listener như test hook hiện có).
  - ✅ **Ngay lập tức có ích kể cả khi chưa làm SDK** — Recommend P3 (`product.viewed`),
    abandoned-cart, analytics cắm được liền.

**E1 — Contract `Plugin` + `PluginManager` (discovery + allow-list + ordering).** *module Hook* — ✅ **đã làm (2026-06-26)**
  - ✅ Contract `Modules\Hook\Plugin\Plugin` (id/version/requires + register/boot + lifecycle
    install/activate/deactivate/uninstall — lifecycle để E2 driven bằng artisan, **không** gọi
    lúc boot) + `BasePlugin` (no-op defaults → plugin chỉ cần id/version/register/boot).
  - ✅ `PluginManager`: **discover** từ (a) composer `extra.lunar-sme.plugin` (đọc
    `vendor/composer/installed.json`) + (b) `config('plugins.paths')` quét `*/*/plugin.json`;
    **allow-list** `config('plugins.enabled')` (chỉ load id có trong list → drop-in nhưng
    không tự chạy code chưa duyệt); **topo-sort** theo `requires` (dep trước dependent, phát
    hiện cycle → log + break); **validate version** bằng `composer/semver` (`Semver::satisfies`,
    `core` = `config('plugins.core_version')`); **fail-soft** — thiếu dep / lệch version /
    class lỗi → `Log::warning` + skip, **không sập app**.
  - ✅ Nạp trong `ModulesServiceProvider`: `PluginManager::load()` chạy **sau** module lõi
    (trước Lunar panel → plugin góp Filament page qua `AdminPages` như module), `boot()` chạy
    trong `ModulesServiceProvider::boot()`. Singleton + `mergeConfigFrom` ở `HookServiceProvider`.
  - ✅ Test `PluginManagerTest` (7 case): discover từ manifest, chỉ load allow-list, off-list
    không load, dep load trước dependent (enable ngược thứ tự vẫn đúng), thiếu dep → skip +
    cảnh báo, core `^2.0` không tương thích → skip + cảnh báo, boot chạy + hook plugin bắn live.
    Fixtures ở `tests/Fixtures/plugins/` (manifest) + `tests/Fixtures/Plugins/` (class).
    **138 test xanh** (+7). Verify thêm: app boot với 0 plugin enabled → `load()` trả `[]`,
    composer discovery trên `installed.json` thật (11k class) → `[]`, không lỗi.

**E2 — Lifecycle + CLI.** *module Hook* — ✅ **đã làm (2026-06-26)**
  - ✅ Bảng `plugins` (id, version, active, installed_at) + model `PluginState`. **Nguồn
    quyết định load vẫn là `config.enabled`** (allow-list trong git); bảng chỉ lưu **state
    cài đặt** để lifecycle idempotent (biết đã install/migrate chưa).
  - ✅ Orchestration trong `PluginManager`: `install()` (record state + chạy `install()` lần
    đầu rồi `activate()`, re-install chỉ re-activate), `disable()` (deactivate, giữ data),
    `uninstall()` (deactivate + `uninstall()` + xoá record), `status()` (cho `plugin:list`).
  - ✅ 4 artisan command (`Modules\Hook\Console\*`): `plugin:list`, `plugin:install`,
    `plugin:disable`, `plugin:uninstall {--force}` (uninstall hỏi xác nhận). Đăng ký trong
    `HookServiceProvider` (chỉ console). `plugin:list` chịu được khi bảng `plugins` chưa
    migrate (degrade = chưa-cài, không crash trên app mới). Verify thật: 4 lệnh hiện trong
    `artisan list`, `plugin:list` render bảng đúng (acme/reviews: enabled=no).
  - ✅ Test (+7 trong `PluginManagerTest`): install record state + chạy lifecycle 1 lần
    (idempotent), install unknown → fail sạch, disable giữ data, uninstall cleanup + drop
    record, `status` đúng cờ enabled/installed/active, `plugin:install` command chạy lifecycle,
    `plugin:uninstall` không `--force` + decline → abort.

**E3 — Plugin tham chiếu (dogfood) + tài liệu.** *plugin mẫu* — ✅ **đã làm (2026-06-26)**
  - ✅ **`plugins/acme/reviews/`** — Review sản phẩm (Lunar không có), dựng **hoàn toàn trên
    SDK + hooks, ZERO sửa lõi**: bảng `product_reviews` riêng + migration (chạy qua `install()`),
    `ReviewsPlugin` (provider) + `ReviewService` + `ReviewController` + `routes/api.php`
    (`GET/POST /api/v1/products/{product}/reviews`), và khối `reviews` (count+average) nhúng
    vào **mọi** product payload qua FILTER `product.resource` (ProductResource **không** biết
    plugin tồn tại). PSR-4 `Acme\Reviews\` → `plugins/acme/reviews/src/`.
  - ✅ Test `ReviewsPluginTest` chứng minh end-to-end: install → load → boot, endpoint hoạt
    động, payload product mang `reviews` qua filter (1 test, không nằm trong `enabled` mặc
    định nên không ảnh hưởng test khác). **146 test xanh** (+8 cho E2+E3).
  - ✅ **`PLUGIN_SDK.md`**: phạm vi (nội bộ, không SaaS), cấu trúc plugin, contract `Plugin`/
    `BasePlugin`, config + lifecycle + 4 CLI, điểm mở rộng, **danh sách đầy đủ `Hooks::*`**,
    plugin tham chiếu.

**E4 — Payload versioning + hardening (khi có >1 plugin thật).** *module Hook* — ✅ **đã làm (2026-06-26)**
  - ✅ **Plugin thứ 2 (Pre-order)** `plugins/acme/preorder/` — đặt hàng trước: bảng
    `product_preorders` riêng; bật pre-order → lật variant sang `always` (backorder native
    Lunar) để mua-khi-hết-hàng qua đúng pipeline Lunar + Inventory guard (không fight pipeline),
    + khối `preorder` (enabled+expected_at) vào payload qua FILTER `product.resource`. Test
    `PreorderPluginTest` (3 case). → giờ có **2 plugin thật** cùng enrich `product.resource`
    (reviews+preorder) + Inventory availability → đủ lý do versioning.
  - ✅ **Payload contract** `Modules\Hook\Support\PayloadContract` (`REQUIRED_KEYS` cho
    product/cart/order + `VERSION='1.0'`) + test `PayloadContractTest` ghim key lõi vào
    response thật (product/cart) → filter/refactor xoá key lõi **fail CI**; có case mô phỏng
    plugin xoá key → contract bắt được.
  - ✅ **`plugin:doctor`** (read-only): in `PayloadContract::VERSION` + báo plugin enabled có
    `requires` không thoả (missing dep / lệch version / không discoverable). `diagnose()` trong
    `PluginManager` + 4 test (healthy / thiếu-dep / core lệch / exit-code command). Verify thật:
    `plugin:doctor` → "All enabled plugins are healthy", `plugin:list` thấy cả 2 plugin.
  - **156 test xanh** (+10 cho E4+preorder).

> ✅ **Hệ sinh thái mở rộng (E0–E4) hoàn tất (2026-06-26).** Domain events đầy đủ (15 hook)
> + Plugin SDK (contract/manager/lifecycle/CLI/doctor) + payload contract versioned + **2
> plugin thật** (reviews, preorder) chạy **zero sửa lõi**. `120 → 156 test`. Tài liệu
> `PLUGIN_SDK.md`. Đúng phạm vi: mở rộng **nội bộ** cho dev, **không** SaaS/marketplace.

**E5 — Trang quản lý plugin trong admin (Filament).** *module Hook* — ✅ **đã làm (2026-06-26)**
  - ✅ **`PluginsPage`** (Filament Page, nav group Settings, slug `/lunar/plugins`): bảng
    quản lý mọi plugin discover được (Version/Enabled/Installed/Active/Requires) + nút
    Install/Reinstall/Disable/Uninstall (gọi `PluginManager`, có `wire:confirm` cho uninstall).
  - ✅ **Mỗi plugin tự chèn tab config:** contract optional **`PluginConfig`** (`configLabel/
    configSchema/configState/saveConfig`) — tách khỏi `Plugin` để giữ contract lõi tối giản.
    `PluginsPage` gom plugin enabled có `PluginConfig` → render **một tab/​plugin** (form
    Filament native, state namespaced theo id qua `statePath` → không đụng nhau), 1 nút Save
    gọi `saveConfig` từng plugin. `PluginManager::configPlugins()` resolve theo `config.enabled`
    hiện tại (không phụ thuộc thời điểm boot).
  - ✅ **`PluginSettings`** (key-value per-plugin, cột `plugins.settings` JSON) + migration
    thêm cột. Reviews implement tab "Auto-approve reviews" (dùng thật trong `ReviewService::add`),
    Preorder tab "Badge label" (vào payload `preorder.label`).
  - ✅ Test `PluginsAdminPageTest` (4 case): liệt kê plugin, tab do plugin chèn, save persist
    per-plugin config (reviews auto_approve=false + preorder label), action install/disable từ
    trang. **160 test xanh** (+4). Verify: page registered trong panel, view `hook::filament.plugins`
    resolve, slug `plugins`.

> **Thứ tự ưu tiên:** **E0 làm trước & độc lập** (giá trị tức thì, không cần SDK). E1–E2
> chỉ làm khi **thực sự cần plugin drop-in của bên thứ ba** — nếu chỉ team nội bộ mở rộng,
> module first-party + E0 đã đủ; **đừng over-engineer SDK sớm** (đúng tinh thần "không build
> sớm" của plan). E3 là cổng nghiệm thu: SDK chỉ "xong" khi một plugin thật chạy không đụng lõi.
